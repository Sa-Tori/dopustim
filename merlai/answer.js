...
var warns = JSON.parse(fs.readFileSync("./data.json", "utf8")); 
...
client.on("message", (msg) => {
    ...
    if (msg.content.startsWith("MEH")) {
            msg.channel.send("```md\n# Празднуйте свое уничтожение.```");             //"```diff\n- Тебе расчесать?```");
    }
...
client.on("message", (msg) => {
    ...
    if (msg.content === "Что??" && msg.author.id === "523116257390886954" && warns == 2) {
        msg.channel.send('Чего тебе, б*я???');
        warns: 3
    };
    if (msg.content === "Привет" && msg.author.id === "542663623789641729" && warns == 3) {
        warns: 0
    };
    if (msg.content.toLowerCase().indexOf("зажги") != -1) {
        msg.channel.send('<a:fire1:768469943288332299><a:fire_blue:768469879211687936><a:fire_green:768469897398190081><a:fire_purple:768469924401250325><a:fire_green:768469897398190081><a:fire_blue:768469879211687936><a:fire1:768469943288332299>');
    }
    if (msg.content.toLowerCase().indexOf("гиф") != -1) {
        msg.channel.send('<a:oanimebooty:685559582550130752>');
    }
...
client.login("NTIzMTE2MjU3MzkwODg2OTU0.XBOj-g.NLKtKBz4kZh7sTZ4HVbD-pvi5V4");
